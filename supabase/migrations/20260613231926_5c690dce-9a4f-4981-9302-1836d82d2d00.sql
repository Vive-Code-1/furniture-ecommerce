...
DROP FUNCTION IF EXISTS public.create_order(text, text, text, uuid, jsonb, text);
...
BEGIN
  ...
  INSERT INTO orders (customer_name, customer_email, customer_phone, shipping_address, total_amount, status, user_id, coupon_code, discount_amount)
  VALUES (trim(p_customer_name), p_customer_email, NULLIF(trim(COALESCE(p_customer_phone,'')), ''), p_shipping_address, v_total, 'pending', p_user_id, v_applied_code, v_discount)
...
REVOKE ALL ON FUNCTION public.create_order(text, text, text, uuid, jsonb, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_order(text, text, text, uuid, jsonb, text, text) TO anon, authenticated;
